...
function getfirst(f, A, default = nothing)
    idx = findfirst(f, A)
    return isnothing(idx) ? default : A[idx]
end
...
function read2dimarray(path, param, timerange = nothing)
    ...
    metadata = Dict(pairs(getfirst(p -> p.mnemonic == uppercase(param), meta_data_params)))
    ...
    elseif haskey(params_2d, param)
        ...
        ch_energy_meta = getfirst(p -> p.mnemonic == uppercase("ch_energy"), meta_data_params)
        dset = params_2d[param]
        # dset = JLD2.get_dataset(jld_f, "Data/Array Layout/2D Parameters/$param")
        dims = (Ti(time), Y(ch_energy; metadata = ch_energy_meta))
    end
