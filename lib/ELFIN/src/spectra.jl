macro sum(ex...)
    # Add fastmath = false to the expression
    return esc(
        quote
            @tullio $(ex...) fastmath = false grad = false tensor = false
        end
    )
end

# It will mutate solid_angle
function sum_valid!(S, dΩ_valid, solid_angle)
    @sum flux[t, E] := _finite(S[t, p, E]) * dΩ_valid[t, p]
    @sum solid_angle[t, E] = dΩ_valid[t, p] * !isnan(S[t, p, E])
    return flux ./= solid_angle
end

# Fast when nan are replaced with 0
function sum_valid_fast!(S, dΩ_valid, solid_angle)
    @sum flux[t, E] := S[t, p, E] * dΩ_valid[t, p]
    @sum solid_angle[t, E] = dΩ_valid[t, p] * iszero(S[t, p, E])
    return flux ./= solid_angle
end

para_losscone(l) = min(l, 180 - l)
is_parallel(p, l, atol) = p < (para_losscone(l) - atol)
is_anti_parallel(p, l, atol) = p > (180.0 + atol - para_losscone(l))
is_perpendicular(p, l, atol) = (pl = para_losscone(l); (pl + atol) < p < (180.0 - pl - atol))
_finite(x::T) where {T} = ifelse(!isnan(x), x, zero(T))


"""
    Espectra(spec_data, time_var, pitch_angles, loss_cone, energy_bins; para_tol=22.25, anti_tol=22.25)

Process 3D spectral data (time × pitch_angle × energy) to extract directional flux spectra.
This implements the same logic as pyspedas epd_l2_Espectra function.
"""
function Espectra(S, pitch_angles, loss_cone; half_sector_width, para_tol = 0, perp_tol = 0)
    n_time, n_pa, n_energy = size(S)
    T = promote_type(eltype(S), eltype(pitch_angles), eltype(loss_cone))
    return @no_escape begin
        Ω = @alloc(T, n_time, n_energy)
        dΩ = @alloc(T, n_time, n_pa)
        dΩ_valid = @alloc(T, n_time, n_pa)

        # Calculate differential solid angle elements for each pitch angle bin
        # Special handling for edge sectors
        in_edge(pa) = pa < half_sector_width || pa > (180 - half_sector_width)
        Δθ = deg2rad(half_sector_width)
        dΩ_edge = Δθ * sin(Δθ)
        @sum dΩ[t, pC] = in_edge(pitch_angles[t, pC]) ? dΩ_edge : 2Δθ * sind(pitch_angles[t, pC])

        # 1. Omnidirectional flux
        omni = sum_valid!(S, dΩ, Ω)
        # 2. Parallel flux
        @sum dΩ_valid[t, pC] = dΩ[t, pC] * is_parallel(pitch_angles[t, pC], loss_cone[t], $para_tol)
        para = sum_valid!(S, dΩ_valid, Ω)
        # 3. Antiparallel flux
        @sum dΩ_valid[t, pC] = dΩ[t, pC] * is_anti_parallel(pitch_angles[t, pC], loss_cone[t], $para_tol)
        anti = sum_valid!(S, dΩ_valid, Ω)
        # 4. Perpendicular flux
        @sum dΩ_valid[t, pC] = dΩ[t, pC] * is_perpendicular(pitch_angles[t, pC], loss_cone[t], $perp_tol)
        perp = sum_valid!(S, dΩ_valid, Ω)

        (; omni, para, anti, perp)
    end
end

function PAspectra(S, dE, min_channels, max_channels)
    n_time, n_pa, n_energy = size(S)
    T = promote_type(eltype(S), eltype(dE))
    # Allocate working arrays for this energy range
    numerator = similar(S, T, n_time, n_pa)
    denominator = similar(S, T, n_time, n_pa)
    return PAspectra!(S, dE, min_channels, max_channels, numerator, denominator)
end

function PAspectra!(S, dE, min_channels, max_channels, numerator, denominator)
    return map(min_channels, max_channels) do min_ch, max_ch
        dE_valid = @view dE[min_ch:max_ch]
        S_valid = @view S[:, :, min_ch:max_ch]
        @sum numerator[t, p] = _finite(S_valid[t, p, e]) * dE_valid[e]
        @sum denominator[t, p] = dE_valid[e] * isfinite(S_valid[t, p, e])
        numerator ./ denominator
    end
end
