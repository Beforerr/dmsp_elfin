...
"""
    get_ae_bin(ae_value::Real, ae_bins::Vector{String}) -> String
...
```
"""
function get_ae_bin(ae_value::Real, ae_bins::Vector{String})
    for bin in ae_bins
...
"""
function interpolate_parameters(model::EmpiricalFluxModel, mlat::Real, mlt::Real, ae::Real;
                                stat::Symbol=:median)
    ...
    distances = map(eachrow(df_ae)) do row
        ...
        Δmlt = min(abs(row.mlt_bin - mlt_norm),
