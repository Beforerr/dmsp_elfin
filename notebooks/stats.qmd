...
sdf = @chain df begin
    ...
    @rsubset!(20 > :κ > 0, :E_c1 < 100, :E_c2 < 100, :maxAE <= 2000)
end
...
for (col_idx, var) in enumerate(vars)
    ...
    # fgrids = draw!(f[1, col_idx], plt)
    # for axe in fgrids
    #     axe.axis.xticks = (1:length(unique(tdf_spatial.mlt_bin)),
    #         ["13", "15", "17", "19", "21", "23", "1", "3", "5", "7", "9", "11"])
    #     axe.axis.yticks = (1:length(unique(tdf_spatial.mlat_bin)),
    #         string.(_mlat.(unique(tdf_spatial.mlat_bin))))
    # end
