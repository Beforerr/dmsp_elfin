...
sdf = let Emin = 0.03
    @chain parameters(df) begin
        ...
        @transform @astable begin
            ...
            :JE_e30 = (e_flux(30, 1000) ∘ m2).(:model) .+ (e_flux(30, 1000) ∘ m1).(:model)
            :J_tot = :J1 + :J2
            :JE_tot = :JE1 + :JE2
            :R_J2 = :J2 ./ :J_tot
            :R_JE2 = :JE2 ./ :JE_tot
...
begin
    ...
    row = @rsubset(sdf, :Δt == Millisecond(0), :Δmlt < 0.05)[3, :]
    ...
    demo_plot(f, trange, tdf, elx_flux, elx_mlt, elx_mlat, dmsp_flux, dmsp_mlt, dmsp_mlat; mlats, add_ratios=false)
    f
end
``` 
...
![energy_spectra_mlt_mlat](../figures/energy_spectra_mlt_mlat.png)
...
```{julia}
for func in (median, mean)
    let vars = [:JE_e30 => L"(e) $JE_{E>30}$: Energy flux (E>30 keV)", :R_JE_e30 => L"(f) $JE_{E>30} / JE_{total}$"]
