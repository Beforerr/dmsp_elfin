...
```{julia}
using TimeseriesUtilities: find_continuous_timeranges
...
valid_ranges_with_ids = map(continuous_ranges) do ranges
    ...
    valid_ids = Int[]
    foreach(dmsp_ids) do id
        dmsp_mlt, dmsp_mlat = get_mlt_mlat(extend(ranges, Δt), id)
        # if any mlat and mlt difference is less than 8 and 1 respectively, push the id to valid_ids
        # Note that dmsp_mlt and elx_mlt are of different sizes
...
df, elfin, dmsp = workload(result...; elx_gei, elx_flux)
```
...
if nrow(df) > 0
    tdf = @rsubset(df, mlt_dist(:dmsp_mlt, :elfin_mlt) < Δmlt_max, :success)
    # random select three mlats
    mlats = sort!(sample(tdf.mlat, min(3, nrow(tdf)), replace=false))
    plot_flux_analysis(tdf, elfin, dmsp; mlats)[1]
else
