...
valid_ranges_with_ids = find_matched_mlat_conditions(
    continuous_ranges, elx_gei; ids=dmsp_ids, Δt, Δmlt_max
)
```
...
dfs = map(valid_ranges_with_ids) do (trange, ids)
    df = try
        ...
        @warn "Failed to process $trange with $ids"
        rethrow()
    end
end
#  (2021-02-19T12:43:00.996538880, 2021-02-19T12:49:01.571536128) with [18]
df = reduce(vcat, filter(!isempty, dfs))
```
...
γ_len = @optic _.model1.γ
κ_len = @optic _.model2.κ
```
