...
for x in (elx_flux.para, elx_flux.anti, elx_flux.prec, dmsp_flux)
    set_if_valid!(x.metadata, :yscale => log10, :ylabel => "Energy (keV)")
end
...
f = Figure()
ax = Axis(f[1, 1]; xscale=log10, yscale=log10, xlabel=𝒀.E, ylabel=𝒀.nflux)
vlines!(ax, Emin; label="Transition Energy", color=:black, linestyle=:dash)

scatterlines!(ax, ee, ff; label="Data", markersize=6)

# Plot the combined model
Eplot = 10 .^ range(log10(minimum(ee)), log10(maximum(ee)), 100)
lines!(ax, Eplot, model.(Eplot); label="Combined Model", linewidth=3, color=:red)

# Plot individual components
lines!(ax, Eplot, model.model1.(Eplot); label="PowerLawExp", linestyle=:dot, color=:blue)
E_high = Eplot[Eplot.>=Emin]
...
@with successful_fits begin
    ...
    plot_parameters_variation(f, :mlat, :model, :n_points; scores = :score)
end
```

Plotting model-calculated flux vs MLat:
...
p1 = plot_flux_by_mlat(f[1, 1], elx_flux.prec, elx_mlat, zoom_tr; colorrange)
p2 = plot_flux_by_mlat(f[2, 1], dmsp_flux, dmsp_mlat, trial_tr; colorrange)
p3 = plot_flux_by_mlat(f[3:4, 1], modeled_fluxes; colorrange)
Colorbar(f[1:4, 2], p3.plot; label=𝒀.nflux)
...
@with successful_fits begin
    plot_parameters_variation(f[1:4, 3:4], :mlat, :model, :n_points; scores = :score)
end
