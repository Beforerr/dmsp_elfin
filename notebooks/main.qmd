...
scatterlines!(ax, ee, f1.(ee); label="PowerLawExp", linestyle=:dot)
scatterlines!(ax, δEs, f2.(δEs); label="SmoothBrokenPowerlaw", linestyle=:dot)
f
```
...
@rtransform! join_df $AsTable = fit_row_parameters(:flux, :flux_1; mlat=:mlat)
# Filter successful fits
# successful_fits = filter(r -> r.success, fit_results)
# @info "Successfully fit $(length(successful_fits)) out of $(length(fit_results)) MLT values"
```
...
# Plot 1: Number of data points vs MLT
ax1 = Axis(f[1, 1], xlabel="MLAT", ylabel="Number of Data Points",
    title="Data Quality vs MLAT")
scatterlines!(ax1, join_df.mlat, join_df.n_points, markersize=8)

plot_example_fits(f[1, 2], join_df)

# Plot 3: PowerLawExp Parameter variation plots
successful_fits = filter(r -> r.success, join_df; view=true)

plot_PowerLawExp_parameter_variation(f[1, 3], successful_fits.mlat, getindex.(successful_fits.params, 1))
...
Analyze how the PowerLawExp parameters change over MLT.
...
```{julia}
# Create comprehensive parameter plots using refactored function
f_detailed = Figure(size=(1600, 1200))
plot_parameters_variation(f_detailed, successful_fits.mlat, successful_fits.params, successful_fits.n_points)
f_detailed
```
