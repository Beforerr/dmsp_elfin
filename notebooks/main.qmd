...
import DmspElfinConjunction.PlotAxis as 𝑨
using Speasy
using SPEDAS
using SPEDAS: tplot
using GLMakie
...
# Load ELFIN EPD data
elx_flux = get_elfin_precipitating_flux(trange, probe)
elx_gei = get_elfin_gei(trange, probe)
elx_gei = tinterp(elx_gei, elx_flux.anti)
elx_aacgm = gei2aacgm(elx_gei)
elx_mlat = elx_aacgm.mlat

dmsp_flux = dmsp_load(trange, 16, "el_d_flux")
dmsp_aacgm = dmsp_get_aacgm(trange, 16)
dmsp_mlat = getindex.(dmsp_aacgm, 1)
dmsp_flux = set(dmsp_flux, Y => dims(dmsp_flux, Y).val .* 1e-3)
dmsp_flux *= 1e6
set_if_valid!(dmsp_flux.metadata,
...
elx_mlat = SPEDAS.set_meta(elx_mlat, :label => "ELFIN")
set_if_valid!(dmsp_mlat.metadata, :label => "DMSP")
...
for da in (dmsp_flux, elx_flux.prec)
    set_if_valid!(da.metadata, :colorrange => (1e3, 1e11))
end
...
f2 = row.flux_1
flux = vcat(f1, f2)
best_result, best_Emin, best_ssr = fit_flux_two_step(flux)
(Es1, m1), (δEs, m2), flux_modeled = best_result
...
scatterlines!(ax, f2)
vlines!(best_Emin)
lines!(ax, Es1, m1.(Es1); label="PLEC", linestyle=:dot)
lines!(ax, δEs, m2.(δEs); label="SBPL", linestyle=:dot)
f
```
